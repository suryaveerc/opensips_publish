...
int build_str_hdr(subs_t* subs, int is_body, str* hdr, str* extra_hdrs) {
	...
	len = 7 /*Event: */+ subs->event->name.len + 4 /*;id=*/+ subs->event_id.len
			...
			+ ((subs->status == TERMINATED_STATUS) ?
					(10/*;reason=*/+ subs->reason.len) :
					9/*expires=*/+ lexpire_len) + CRLF_LEN
			+ (is_body ?
	...
	*(p++) = '>';
	...
	if (is_body && subs->event->content_type.s
...
*/
	...
	done: notify_body = create_winfo_xml(watchers, version_str, subs->pres_uri,
...
static inline db_res_t* pres_search_db(struct sip_uri* uri, str* ev_name,
		int* body_col, int* extra_hdrs_col, int* expires_col, int* etag_col) {
	/*	static db_ps_t my_ps = NULL; */
...
//select body,extra_hdrs,expires,etag from presentity where domain=? AND username=? AND event=? order by received_time
	...
	*/if (getResource(query_cols, query_vals, n_query_cols, &result, PRESENTITY,
					uri->user.s, NULL, result_cols, n_result_cols) < 0) {
...
str* get_p_notify_body(str pres_uri, pres_ev_t* event, str* etag,
	...
	*tk = '\0';
	...
	found_in_cache = fetchPresentityFromCache(p_key, &body_col,
			&extra_hdrs_col, &expires_col, &etag_col, &result);
	...
	if (!found_in_cache) {
		...
		result = pres_search_db(&uri, &event->name, &body_col, &extra_hdrs_col,
...
*/
		...
		if (event->agg_nbody) {
			...
			if (body_cnt
					== 0&& (mix_dialog_presence||notify_offline_body) && event->evp->parsed == EVENT_PRESENCE) {
			...
			notify_body = event->agg_nbody(&uri.user, &uri.host, body_array,
					body_cnt, -1);
			if (notify_body) {
				goto done;
			}
        ...
        if (event->agg_nbody == NULL) {
			...
			if (row_vals[extra_hdrs_col].val.string_val != NULL) {
				if (extra_hdrs && !extra_hdrs->s) {
					...
					memcpy(extra_hdrs->s,
							row_vals[extra_hdrs_col].val.string_val, len);
					extra_hdrs->len = len;
				}
			}
...
//		result= NULL;
		...
		if (event->evp->parsed == EVENT_DIALOG && from_publish && publ_body) {
			 ...
			 */
			notify_body = event->agg_nbody(&uri.user, &uri.host, &publ_body, -2,
					build_off_n);
			if (notify_body) {
				goto done;
			}
	...
	done: if (body_array != NULL && body_array != &dialog_body) {
...
dlg_t* build_dlg_t(subs_t* subs) {
	...
	error: free_tm_dlg(td);
	return NULL;
}

int get_subs_db(str* pres_uri, pres_ev_t* event, str* sender, subs_t** s_array,
		int* n) {
...
int update_in_list(subs_t* s, subs_t* s_array, int new_rec_no, int n) {
	...
	for (i = 0; i < n; i++) {
		...
		if (ls->callid.len == s->callid.len
				...
				&& strncmp(ls->from_tag.s, s->from_tag.s, s->from_tag.len)
						== 0) {
...
subs_t* get_subs_dialog(str* pres_uri, pres_ev_t* event, str* sender) {
	...
	error: free_subs_list(s_array, PKG_MEM_TYPE, 0);
	return NULL;
}
...
int publ_notify(presentity_t* p, str pres_uri, str* body, str* offline_etag,
	...
	while (s) {
		...
		if (notify(s, NULL, notify_body ? notify_body : body, 0,
				p->extra_hdrs ? p->extra_hdrs : &notify_extra_hdrs,
				from_publish) < 0) {
...
int send_notify_request(subs_t* subs, subs_t * watcher_subs, str* n_body,
			...
			if (subs->event->type & WINFO_TYPE) {
				...
				if (from_publish && n_body != 0 && n_body->s != 0)
					...
					notify_body = get_p_notify_body(subs->pres_uri, subs->event,
							0, 0, (subs->contact.s) ? &subs->contact : NULL,
...
int notify(subs_t* subs, subs_t * watcher_subs, str* n_body,
	...
	if (subs->expires != 0 && subs->status != TERMINATED_STATUS) {
		...
		if (updateSubscriptionInCache(subs, LOCAL_TYPE) < 0) {
			//LM_DBG("record not found in cache\n");
		}
	...
	if (send_notify_request(subs, watcher_subs, n_body, force_null_body,
					extra_hdrs, from_publish) < 0) {
