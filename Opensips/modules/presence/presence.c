...
int avail_subs=0; // temp variable to store status of subscriptions. IF available 1 else 0.
int avail_pubs=0; // temp variable to store status of publication. IF available 1 else 0.
char *cachedb_host;
char *pub_queue_subject;
char *sub_queue_subject;
int cachedb_port;
char *ROOT_URL;
...
int micro_srv_arch; // switch between sending to queue or to notify within the code.
CURL *curl;
...
static param_export_t params[] = {
        {"micro_srv_arch",        INT_PARAM, &micro_srv_arch},
        {"available_subs", INT_PARAM, &avail_subs},
        {"available_pubs", INT_PARAM, &avail_pubs},
        {"pub_queue_subject",     STR_PARAM, &pub_queue_subject},
        {"sub_queue_subject",     STR_PARAM, &sub_queue_subject},
...
//	LM_DBG("db_url=%s/%d/%p\n", ZSW(db_url.s), db_url.len, db_url.s);
    ...
    if (micro_srv_arch) {
        ...
        if (queue_connect((const char **) queueServerUrls) == -1)
            return -1;
        setQueue(pub_queue_subject, sub_queue_subject);
